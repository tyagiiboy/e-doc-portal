...
const Header = (props) => {
  ...
  const signOutHandler = () => {
    props.setNavMenuState(state => !state)
  ...
  return (
    <header className='bg-teal-700 h-12 sticky top-0 z-20 p-1'>
      <section className='max-w-4xl  mx-auto flex justify-between'>
        ...
        <nav className="">
          <button id="dropdownDefaultButton" className={`${(props.navMenuState)? '': 'hidden'} text-white bg-teal-700 hover:bg-teal-800 font-medium rounded-lg text-sm px-4 py-2.5 text-center inline-flex items-center`} onClick={onClickHandle} type="button">Dropdown options<svg className="w-4 h-4 ml-2" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></button>
